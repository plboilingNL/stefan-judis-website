<template>
  ...
  <div class="c-preview">
    ...
    <div v-if="item.sys.contentType.sys.id === '2wKn6yEnZewu2SCCkus4as'" class="u-marginTopAuto">
      ...
      <p v-if="!item.fields.externalUrl"><nuxt-link :to="`/${linkPrefix}/${item.fields.slug}/`" class="o-btn" :aria-labelledby="item.fields.title | idAlize">Read more</nuxt-link></p>
    </div>

    <div v-if="item.sys.contentType.sys.id === 'talk'" class="u-marginTopAuto">
      <p><a :href="item.fields.slideUrl" class="o-btn" :aria-labelledby="item.fields.title | idAlize">Check the slides</a></p>
    </div>

    <div v-if="item.sys.contentType.sys.id === 'project'" class="u-marginTopAuto">
      <p><a :href="item.fields.url" class="o-btn" :aria-labelledby="item.fields.title | idAlize">Check the project</a></p>
    </div>
